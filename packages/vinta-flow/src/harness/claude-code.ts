 ...
 * then `VINTA_FLOW_CLAUDE_BIN`, then `"claude"`.
 *
...
export interface ClaudeCodeAdapterOptions {
  /** Overrides `VINTA_FLOW_CLAUDE_BIN`, which overrides `"claude"`. */
  readonly bin?: string
...
const INSTALL_HINT =
  'npm install -g @anthropic-ai/claude-code (or point VINTA_FLOW_CLAUDE_BIN at the binary)'
...
export class ClaudeCodeAdapter implements HarnessAdapter {
  ...
  constructor(private readonly options: ClaudeCodeAdapterOptions = {}) {
    this.bin = options.bin ?? process.env['VINTA_FLOW_CLAUDE_BIN'] ?? 'claude'
  }
  ...
  async spawn(task: AgentTask): Promise<SpawnOutcome> {
    ...
    try {
      child = spawnChild(this.bin, args, {
        cwd: task.cwd,
