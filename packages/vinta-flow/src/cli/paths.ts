 ...
 *
 * `.vinta-flow/` is §5.3's store, and §11 is emphatic that it stays *inside the
...
export const storeFor = (repoPath: string): string => join(repoPath, '.vinta-flow')

/** Run directories: `.vinta-flow/runs/<run-id>`. The only thing `purge` removes. */
