 ...
 * distinction is not cosmetic; it is what makes `vinta-flow simulate … > out`
...
export async function loadWorkflow(path: string, io: Io): Promise<Workflow | null> {
  ...
  try {
    ...
    io.err(`vinta-flow: cannot read workflow file: ${path}`)
    return null
  }
  ...
  try {
    ...
    io.err(`vinta-flow: ${path} is not valid JSON`)
    return null
  }
  ...
  io.err(`vinta-flow: ${path} is not a valid workflow (${count} issue${count === 1 ? '' : 's'})`)
