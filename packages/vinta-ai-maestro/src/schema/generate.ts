 ...
 * in `src/types.ts`.
 *
 *   pnpm --filter vinta-ai-maestro schema:gen      write it
 *   pnpm --filter vinta-ai-maestro schema:check    fail if the committed file drifted
 *
...
if (process.argv.includes('--check')) {
  ...
  try {
    ...
    console.error(`missing ${OUT} — run \`pnpm --filter vinta-ai-maestro schema:gen\``)
    process.exit(1)
  }
  if (committed !== serialized) {
    console.error(
      `${OUT} is out of date with src/types.ts — run \`pnpm --filter vinta-ai-maestro schema:gen\``,
    )
    process.exit(1)
  }
