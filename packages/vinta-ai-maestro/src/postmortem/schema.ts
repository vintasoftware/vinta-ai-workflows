...
export function buildPostMortemSchema(): Record<string, unknown> {
  ...
  return {
    ...
    title: 'vinta-ai-maestro plan post-mortem',
    description:
      `${String(description)} Lives at \`.vinta-ai-maestro/runs/<run-id>/postmortem.json\` in a ` +
      'target project, emitted by `vinta-ai-maestro` after a run ends and read by `plan-feature` ' +
      'when planning the next feature in the same repo. GENERATED from ' +
      'packages/vinta-ai-maestro/src/postmortem/postmortem.ts — edit there, not here. See ' +
