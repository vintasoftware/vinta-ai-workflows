...
description: Author project-specific skills under `ai-tools/skills/`. Always copies the project-agnostic foundation set (`plan-feature`, `create-spec`, `open-pr-from-context`) verbatim from this skill's bundled resources; ships `create-qa-use-cases` and keeps `plan-feature`'s e2e sections only when `add-e2e-test` is enabled. Generates `implement-plan` from a parameterized template using project-specific commands + branch / commit / PR conventions captured in the bootstrap interview. Asks the user whether `add-e2e-test`, `add-env-var`, `dev-desk-check` and `qa-review` are needed (optional foundation skills — only ship if applicable). Stack-specific skills (Medplum, Django, etc) come from user-supplied templates pointed at by the orchestrator — see [bootstrap-ai-tools/resources/stacks/<stack>/notes.md](../vinta-bootstrap-ai-tools/resources/stacks/) for the categories per stack.
---
...
- **Replace with Vinta foundation version** (foundation-shape only — `plan-feature`, `create-spec`, `create-qa-use-cases`, `implement-plan`, `add-e2e-test`, `add-env-var`, `add-one-off-script`, `prepare-worktree`, `thermo-nuclear-code-quality-review`, `deslop-comments`, `handoff`, `handoff-to-client`, `write-unit-test`, `dev-desk-check`, `qa-review`) — proceed with the bucket A / B / C flow below for that name. The user has explicitly opted into overwriting their version.
...
### B. Generate — the **plan-execution unit** + `systematic-debugging` + `write-unit-test` from templates
...
- **`write-unit-test`** — write durable unit tests for a unit of behavior, enforcing the universal rules (mock only genuine externals, assert the full expected output not counts/truthiness, clean up created data or roll back the transaction, never hit an external service unavailable in dev/test, keep test logic literal, decouple from internals) plus this project's captured test conventions. From [resources/write-unit-test-template.md](resources/write-unit-test-template.md). **Default-on, not ask-first** — generated whenever `foundation_skills.write-unit-test` is `enabled`, which bootstrap sets automatically when `vinta-analyze-codebase` detected a unit-test framework (`disabled` → skip; nothing to render against). The rendered body bakes in `skills.write-unit-test.*` config: test style, data setup, assertion style, DB isolation, plus any inferred `additional_conventions`. Standalone — no cross-links into the plan-execution unit.

  **Ship the packs too — two axes, matched independently.** Alongside the rendered `SKILL.md`, copy the **verbatim** packs the project needs from [resources/write-unit-test-packs/](resources/write-unit-test-packs/) into `ai-tools/skills/write-unit-test/resources/packs/`. There are two kinds, selected on **different** signals so framework-specific advice only ships when its framework is actually present:

  - **Runner pack** (`resources/packs/runners/<runner>.md` → shipped to `.../packs/runners/`) — the test-runner idioms (structure, assertions, mocking). Select **one (or, in a polyglot repo, one per language)** by `inventory.tests.unit_framework`: `pytest` → `pytest.md`; `vitest` → `vitest.md`; `jest` → `jest.md`. Runner packs are **stack-agnostic** — they contain no Django/FastAPI/Medplum specifics.
  - **Stack pack(s)** (`resources/packs/stacks/<stack>.md` → shipped to `.../packs/stacks/`) — DB isolation, framework client/test-client, domain mocks. Ship **every** pack whose stack the inventory matched (a project can match several — e.g. `nextjs` + `react`, or `tanstack-start` + `react`). Match on `inventory.frameworks` / `inventory.stacks`:

    | Detected signal | Stack pack |
    |---|---|
    | Django | `django.md` |
    | FastAPI | `fastapi.md` |
    | Flask | `flask.md` |
    | Medplum (`@medplum/*`) | `medplum.md` |
    | React (renders UI) | `react.md` |
    | Next.js | `nextjs.md` |
    | TanStack Start (`@tanstack/react-start`) | `tanstack-start.md` |
    | React Router (`react-router` / `@react-router/*`) | `react-router.md` |
    | Prisma (`@prisma/client`) | `prisma.md` |
    | Standalone Python package (no web framework) | `python-package.md` |
    | Standalone TS package / lib (no framework) | `typescript-package.md` |

  **Only ship what matched — never dump the whole directory.** A FastAPI project must not receive `django.md`; a pure-library project gets its `*-package.md` and **no** web-framework pack. When no stack pack matches (e.g. a plain script repo), ship the runner pack alone — the rendered body already falls back to the universal rules + the nearest existing tests. When the runner or a clearly-present stack has no pack here, ship nothing for it and record the gap in the orchestrator's final summary (a candidate for a new pack via the source repo's `add-foundation-skill` flow). The `{{FRAMEWORK_PACK_LOADER_BLOCK}}` + `{{FRAMEWORK_PACKS_LIST}}` placeholders are rendered from exactly the set of packs you shipped.

  **Some stack packs need a one-time project-setup step, done here — not per test.** A pack may require a project-level test-setup fixture that must exist before any test runs; wire it once at derive time so no individual test has to. Currently:

  - **Medplum** (Vitest) — `MockClient` search filtering (`searchResources` / `searchOne` with filters) silently returns nothing unless the FHIR structure definitions + search parameters are indexed in the test process. When you ship `medplum.md`, also install the **two** bundled Vitest setup files and wire both hooks:
    - Copy [resources/write-unit-test-packs/stacks/medplum-test.globalSetup.ts](resources/write-unit-test-packs/stacks/medplum-test.globalSetup.ts) → the project's setup dir (commonly `src/test.globalSetup.ts`) and register it in `vitest.config.*` under `test.globalSetup`. It indexes the bundles once in the main process and shares `globalSchema.types` to every worker via `project.provide`.
    - Copy [resources/write-unit-test-packs/stacks/medplum-test.setup.ts](resources/write-unit-test-packs/stacks/medplum-test.setup.ts) → `src/test.setup.ts` and register it under `test.setupFiles`. It `inject`s the shared index and assigns it at **module scope** (not `beforeAll` — timing matters).
    - **Deps:** `@medplum/definitions` as a devDependency; Vitest **3+** (the `provide`/`inject` API); Node **≥ 22.4** or add `ws` for the WebSocket polyfill (@medplum/core v5). Flag any of these that are missing.
    - **Version alignment:** `@medplum/core`, `@medplum/definitions`, `@medplum/mock`, `@medplum/fhirtypes` must be on matching versions.
    - **Idempotent + merge, don't clobber:** if the project already has `test.setup.ts` / `test.globalSetup.ts`, merge the indexing + inject/provide logic in rather than overwriting; if it already does this, do nothing. Create the files + point the runner config at them only when absent.
    - **Jest-based Medplum project** (rare — the catalog is Vitest-first): there's no bundled Jest variant; index the FHIR bundles in a single `setupFilesAfterEnv` file instead and record it as a follow-up to add a Jest snippet to the catalogue.

    This is a project mutation — do it once at bootstrap. Canonical source is the team's snippet catalogue (`vintasoftware/medplum-snippet-catalog`, `TestSetup/`); the bundled copies (dated in their headers) keep bootstrap offline-safe — prefer the live catalogue if it has advanced. The rendered `medplum.md` only *mentions* the override so tests don't re-index.

- **`dev-desk-check`** — drive a real browser through the app as a user, judge what renders, and produce evidence someone who wasn't there can trust. Plan source is the branch diff (vs the PR's real base), ids from the project's QA use-cases doc, or a ticket. From [resources/dev-desk-check-template.md](resources/dev-desk-check-template.md). **Opt-in** — only generated when `foundation_skills.dev-desk-check` is `enabled`. The rendered body bakes in `skills.dev-desk-check.*`: environments with their per-environment write policy and data sensitivity, optional service probes, auth account roles (pointers, never secrets), feature-flag system, design source. Composes with, and never duplicates, its neighbours — [create-qa-use-cases](resources/foundation-skills/create-qa-use-cases/SKILL.md) writes the checklist, this walks it; `add-e2e-test` automates a use case, this reports which ids a change touches. **Not** wired into `implement-plan`'s phase gate; a browser pass per phase is too slow, the same reasoning that keeps `run_options.run_e2e` off.

- **`qa-review`** — the same browser walk and report shape for product managers and designers, stripped of everything that assumes a repo: no git, no diff, no dev servers, no service probes, no PR edit, no database. From [resources/qa-review-template.md](resources/qa-review-template.md). **Opt-in** — only generated when `foundation_skills.qa-review` is `enabled`, independently of `dev-desk-check` (a project may take either or both). Renders from the **same** `skills.dev-desk-check` config block. Rendered **self-contained**: everything it needs is inlined, so the single file works when dropped at `~/.claude/skills/qa-review/SKILL.md` by someone with no checkout. Stable facts are inlined (environment addresses, which account role to use, the design source, who to ask when blocked); the QA use-cases doc deliberately is **not** — a snapshot of a checklist the repo keeps updating is worse than no snapshot, so the reviewer pastes the use case instead. The render is stamped with its generation date and source commit; [vinta-update-project-skills](../vinta-update-project-skills/SKILL.md) re-renders it when the facts change.
...
| `{{FRAMEWORK_PACKS_LIST}}` | derived from the packs shipped (write-unit-test only) | comma-separated names of the runner + stack packs that landed under `resources/packs/`, e.g. `pytest + Django` or `vitest, React, TanStack Start`. Renders `the project's test framework` when only the runner shipped with no stack pack. |
| `{{FRAMEWORK_PACK_LOADER_BLOCK}}` | derived from the packs shipped (write-unit-test only) | bullets pointing at each shipped pack, runner first then stacks: `- Runner: **pytest** → read [resources/packs/runners/pytest.md](resources/packs/runners/pytest.md).` then `- Stack: **Django** → read [resources/packs/stacks/django.md](resources/packs/stacks/django.md).` (one bullet per matched stack). When only the runner shipped, emit just its bullet plus `- No stack pack matched — apply the universal rules and mirror the nearest existing tests.` When no pack shipped at all, render that single fallback line alone. |
| `{{PROJECT_TEST_PREFERENCES_BLOCK}}` | derived from `skills.write-unit-test.{test_style,data_setup,assertion_style,db_isolation}` (write-unit-test only) | one bullet per field whose value is **not** `framework-default`, phrased as an instruction: `test_style: classes` → `- Group tests in test classes (not standalone functions).`; `data_setup: factories` → `- Build test data with factory classes / builders, not inline literals or fixture files.`; `assertion_style: plain-assert` → `- Use plain \`assert\` (not \`self.assertEqual\`-family methods).`; `db_isolation: transaction-rollback` → `- Isolate DB state with transaction rollback (each test in a rolled-back transaction).`. Emit `- No project-specific overrides — follow the framework pack's defaults.` when all four are `framework-default`. |
| `{{ADDITIONAL_CONVENTIONS_BLOCK}}` | derived from `skills.write-unit-test.additional_conventions[]` (write-unit-test only) | when non-empty: a `### Additional project conventions` sub-block listing each entry as a bullet, verbatim. Empty string (whole block omitted) when the array is empty. Starts with a leading blank line so it slots cleanly under the preferences block. |
| `{{DEPENDENCY_LICENSE_BLOCK}}` | derived from `policies.dependency_licenses` | Top-level `## Adding new third-party dependencies` section. Empty string when `enforcement: off` or the block is absent. When `block` / `warn`: short paragraph naming the enforcement mode + the forbidden SPDX list + the pre-install check (`npm view <pkg> license`, PyPI metadata, etc.) + a one-line pointer to the **Dependency licenses** section in the root `AGENTS.md` — linked as `../../../AGENTS.md`, the depth from `ai-tools/skills/<name>/SKILL.md` — for the full table including `allowed_overrides` and `notes`. See [resources/dependency-license-block.md](resources/dependency-license-block.md) for the canonical body. |
...
| `{{QA_ENVIRONMENTS_TABLE}}` | `skills.dev-desk-check.environments` (dev-desk-check + qa-review) | Markdown table, one row per declared environment: `\| Name \| Base URL \| Writes \| Data sensitivity \|`. Never empty — the schema requires at least one environment. |
| `{{QA_ENVIRONMENT_NAMES}}` | `skills.dev-desk-check.environments` (dev-desk-check + qa-review) | Comma-separated environment names in declaration order, e.g. `local, staging, production`. **Frontmatter-only in both templates** — the rendered body sources its own option lists from `{{QA_ENVIRONMENTS_TABLE}}` instead, so a check that scans only the body (not the `description:` frontmatter) will find this placeholder unused there; that is expected, not a gap. |
| `{{QA_PREFLIGHT_BLOCK}}` | `environments.<name>.{api_url,api_health_path,frontend_start,edge_proxy}` (dev-desk-check only) | Per-environment preflight steps for **frontend then backend only** — services are rendered separately, above, by `{{QA_SERVICES_BLOCK}}` under its own heading; this block sits under the template's literal `### Frontend and backend` heading and must not re-emit service probes there. **`edge_proxy` decides the shape of an environment's whole entry — frontend and backend together — and it overrides every probe instruction below it.** When `edge_proxy` is set, emit no `curl` at all for that environment and render instead: `<name> sits behind <edge_proxy>, which answers non-browser traffic with a 403. No curl preflight — the browser is the preflight.` Probing an edge-fronted environment reports a healthy app as down, which is the one preflight failure that costs the entire run. When `edge_proxy` is **absent**, emit a frontend probe `curl -sS -o /dev/null -w '%{http_code}' --max-time 10 <base_url>`, followed by a backend probe `curl -sS -o /dev/null -w '%{http_code}' --max-time 10 <api_url><api_health_path>` **only** when `api_url` is set. `frontend_start` governs *starting*, never *probing*: emit the start offer (local-only, asks first) when it is set, and otherwise emit `No start command is declared for <name> — never start a server for it. A second dev server on an occupied port silently invalidates the whole run.` **That fallback must not carry a probe instruction of its own**, or it contradicts the `edge_proxy` rule for every environment that is both edge-fronted and unstartable — which is every remote environment a typical project declares. |
| `{{QA_SERVICES_BLOCK}}` | `skills.dev-desk-check.services[]` (dev-desk-check only) | The whole `### Services pre-check` section body — **h3, because it nests inside `## Phase 3 — Preflight`, not h2** — one bullet per service with its `probe`, its `required` verdict (`true` → stop the run and offer `start`; `false` → continue and record the degradation in the report), and the three fixed rules (probe-cannot-run is `unknown` not `down`; auto-start is local-only and asks first; probes run before the frontend and backend checks, because a dependency that is down produces failures indistinguishable from product bugs and a run that reaches the browser with a dead queue spends its findings section describing the outage). **The hosting template's Phase 3 prose names no services step of its own** — the fixed "Fixed order" line lists `frontend → backend → data preconditions` only, so a project declaring no `services` ships a Phase 3 that does not advertise a step it will never run. Keep it that way: an ordering or rationale sentence about services belongs in this block, which vanishes with them, never in the template around it. **Empty string when `services` is absent or empty — the whole section disappears, heading included.** |
| `{{QA_WRITE_POLICY_BLOCK}}` | `environments.<name>.writes` (dev-desk-check + qa-review) | Renders **only the per-environment write-policy bindings** — one line per declared environment naming its `free` / `confirm` / `forbidden` value and what that value means there. Must **not** re-emit a fixed write-gate rule the hosting template already carries as literal prose: guard rails are data, the seven rules from the spec's **Production write gate** are prose, and a rule shipped twice is worse than shipped once — a reader cannot tell which copy binds. **`dev-desk-check`**: the block supplies **none** of the seven — its template already carries all seven verbatim (rules 1–7 in order), including rule 6's `Writes carry synthetic values only. Never real client or patient data as input. Under `phi`, the ledger records opaque IDs, not field values.` clause. **`qa-review`**: its template already carries rules 1, 3, 4 and 6 in reviewer-language prose, plus the UI-reversal rule as its own bullet — so the block supplies **only rules 2, 5 and 7**, in reviewer language: rule 2 (tracking what changed applies no matter what the answer above was), rule 5 (a change the policy above refuses gets written down and proposed, never made), and rule 7 (anything whose effects reach beyond the one thing being tested is proposed and left for an engineer, in every environment) — avoiding `ledger`, `seed`, `migration`, `pull request` and `branch` in all three. |
| `{{QA_DATA_SENSITIVITY_BLOCK}}` | `environments.<name>.data_sensitivity` (dev-desk-check + qa-review) | The evidence-handling table restricted to the sensitivity levels actually declared, plus one line per environment binding it: `production → phi: opaque ids only, never field values; no Artifact deliverable.` When every environment is `none`, render the one-line form: `No environment carries client or patient data. Screenshots and report contents are unrestricted.` |
| `{{QA_AUTH_BLOCK}}` | `skills.dev-desk-check.auth` (dev-desk-check + qa-review) | Renders **only** the `accounts[]` table — `\| Role \| Credential location \|`, `where` verbatim as a pointer, never a secret. The manual-login rule itself is already fixed template prose, not something this block supplies: `### Manual login` in the dev-desk-check template, and `**You sign in yourself.**` sitting directly next to the placeholder in the qa-review template. When `accounts` is empty: **dev-desk-check** emits `No account roles are declared — ask the operator which account to use.` **qa-review** must not reuse that engineer-facing fallback — its reader *is* the person signing in — and instead addresses them directly, e.g. `No account role is declared for this project — ask {{QA_ESCALATION_CONTACT}} which account to use.` |
| `{{QA_FEATURE_FLAGS_BLOCK}}` | `skills.dev-desk-check.feature_flags` (dev-desk-check + qa-review) | Names the declared `system` and the `check` pointer. **dev-desk-check variant owns its own `### Feature flags` heading** — do not add a literal heading above it. **Empty string when `feature_flags` is absent or `system: none` — the whole section disappears, heading included.** In the `qa-review` render, the block instead brings its **own** heading in reviewer language (e.g. `### Settings that turn features on`), naming the *setting* rather than the flag system behind it — never `waffle` / `launchdarkly` by name. The hosting template's own fixed prose already carries the blocked-by-a-setting outcome in reviewer language — `Blocked by the <name> setting — ask an engineer to enable it` — so this block must **not** re-append a second, differently-worded copy of that line. |
| `{{QA_DESIGN_SOURCE_BLOCK}}` | `skills.dev-desk-check.design_source` (dev-desk-check + qa-review — **two variants**) | `type: figma` → compare against the live node, citing `file_url`. `type: exports` → compare against the designer-supplied exports at `export_dir` and **label them as exports in the report**. **dev-desk-check variant**: empty string when `design_source` is absent or `type: none` — the whole `### The design pass` heading disappears with it, **heading included** — and the skipped pass is recorded under *Not tested* with the reason; names the tooling: "via the Figma MCP tools available at runtime". **qa-review variant is never the empty string** and carries **no heading of its own** — the numbered step heading above it in the template covers that job, so a vanished region would leave a hole in the sequence the reader follows. On `type: none` it renders exactly: `There is no design to compare against on this project. Note that under *Not tested* and move on.` It also never names the tooling — "open the design file at `<file_url>` and compare side by side" instead, because its reader is the designer and "MCP" is engineer vocabulary. This is the same two-variant shape `{{QA_FEATURE_FLAGS_BLOCK}}` and `{{QA_USE_CASES_BLOCK}}` already use, and it is what keeps the rendered `qa-review` inside the jargon ban enforced by the template-only placeholder/jargon grep (and, on the rendered output, the derive-time portability check above). |
| `{{QA_USE_CASES_BLOCK}}` | `skills.dev-desk-check.use_cases_path` + `foundation_skills.create-qa-use-cases` (dev-desk-check + qa-review) | **dev-desk-check:** points at the path and says use-case ids are the strongest plan source because the document already states the steps and the expected outcome. Links `[create-qa-use-cases](../create-qa-use-cases/SKILL.md)` **only when that skill is enabled**. **qa-review:** never inlines the checklist and never links a path — renders `Paste the use case, the ticket, or a description of the feature. The checklist this project keeps at <use_cases_path> changes often; a snapshot of it would be worse than none.` |
| `{{QA_E2E_IMPACT_BLOCK}}` | `foundation_skills.add-e2e-test` (dev-desk-check only) | When `enabled`: a Phase 1 step reporting which `use_cases_path` ids the change touches, and a closing pointer to `[add-e2e-test](../add-e2e-test/SKILL.md)` for automating them. **Empty string when disabled or absent** — the link would dangle. |
| `{{QA_BASE_BRANCH_DETECTION_BLOCK}}` | `project.code_host` + `project.default_branch` (dev-desk-check only) | Host-specific PR-base lookup: `github` → `gh pr view --json baseRefName -q .baseRefName`; `gitlab` → `glab mr view --output json` then read `target_branch`; `bitbucket` / `self-hosted` / `other` → ask the operator. Always followed by the fixed rule: **"no PR" and "the CLI failed" are different answers. Only the first may fall back to `{{DEFAULT_BRANCH}}`. A CLI error stops and reports — a blanket fallback turns an expired token into a plan built from the wrong diff with nothing on screen saying so.** |
| `{{QA_REPORT_DIR}}` | `skills.dev-desk-check.report_dir` (dev-desk-check + qa-review) | Plain value, e.g. `.vinta-ai-workflows/qa`. **In the `qa-review` render this value is the right location only when the reader happens to have a copy of the project on the machine they're working on** — it is a project-relative, deliberately hidden folder, and the normal holder of a `qa-review` file has no checkout at all. Without that check, the render would tell a reader with no repo that a report was saved somewhere they can never reach, and their agent would create the folder wherever it happened to be running. The `qa-review` template's sentence naming this value is therefore conditional on purpose: inside a checkout, the report goes there; without one, it goes wherever it can be handed directly to whoever asked. |
| `{{QA_ARTIFACT_DELIVERABLE_BLOCK}}` | max `data_sensitivity` across declared environments (qa-review only) | `none` → offer the choice between a markdown report and a published Artifact carrying screenshots inline. `client` → offer the Artifact only behind an explicit confirmation naming the environment. `phi` → **do not offer it at all**; render `Markdown only. Publishing to hosted storage would move records outside the managed platform's access controls and audit logging.` Markdown is always available in every case. **This block is the sole authority on where a `qa-review` report may go** — the template's fixed closing line ("this never touches a pull request…") is explicitly bounded by whatever this block offered, so the block itself must carry the actual policy rather than stand in for one. |
| `{{QA_ESCALATION_CONTACT}}` | `skills.dev-desk-check.escalation_contact` (dev-desk-check + qa-review) | The declared contact verbatim. Renders `the engineer who gave you this file` when absent. |
| `{{QA_REVIEW_PROVENANCE_BLOCK}}` | derive-time (qa-review only) | `Generated <YYYY-MM-DD> from {{PROJECT_NAME}} at commit <short-sha> by vinta-derive-skills <package version>.` plus the fixed refresh paragraph naming `vinta-update-project-skills` and telling the holder to ask {{QA_ESCALATION_CONTACT}} for a fresh copy when the facts above look wrong. |
...
- `ai-tools/skills/write-unit-test/SKILL.md` (from `write-unit-test-template.md`) **plus** `ai-tools/skills/write-unit-test/resources/packs/runners/<runner>.md` + `.../packs/stacks/<stack>.md` (verbatim from `write-unit-test-packs/`, only the detected runner + matched stacks) — only when `foundation_skills.write-unit-test: enabled` (default-on when a test framework was detected)
- `ai-tools/skills/dev-desk-check/SKILL.md` (from `dev-desk-check-template.md`) — only when `foundation_skills.dev-desk-check: enabled`
- `ai-tools/skills/qa-review/SKILL.md` (from `qa-review-template.md`) — only when `foundation_skills.qa-review: enabled`. Rendered self-contained; see the portability check below.
...
**`qa-review` carries one extra validation — the portability check.** Its whole value is that one file works with no checkout, so before saving the render, grep it:

```bash
F=ai-tools/skills/qa-review/SKILL.md
grep -nE '\]\(\.\.?/' "$F"
grep -nE '^[[:space:]]*(git|npm|pnpm|yarn|docker|curl|make|pytest|python3?|node|npx)[[:space:]]' "$F"
grep -nE '`(git|npm|pnpm|yarn|docker|curl|pytest|npx)[[:space:]]' "$F"
grep -niE 'reducer|migration|API endpoint|waffle|launchdarkly|playwright|\bMCP\b|ledger|seed|pull request|branch' "$F"   # jargon that can arrive via a rendered block
```

Three greps rather than one alternation, because a single `make ` pattern false-positives on ordinary prose. Any hit means a relative link or a repo-dependent command survived — usually from a config value spliced into a block, not from the template itself. Rewrite the offending value out of the render (or out of the config) before saving. A `qa-review` that links `../create-qa-use-cases/SKILL.md` is a broken file the moment its holder opens it.

The jargon grep's term set is wider here than what a check over the template source alone can enforce, on purpose: `ledger`, `seed`, `pull request` and `branch` are added because three of the blocks that feed this render — `{{QA_WRITE_POLICY_BLOCK}}`, `{{QA_DATA_SENSITIVITY_BLOCK}}`, `{{QA_ARTIFACT_DELIVERABLE_BLOCK}}` — are shared with the engineer-facing `dev-desk-check` path. Jargon can arrive **through a rendered block** even when the template itself is clean, which is exactly what a grep over the template source cannot catch, because it only ever sees `{{...}}` still unexpanded. This derive-time check is where a config value that leaked engineer language into a shared block actually gets caught.
...
| `dev-desk-check` | "Should agents be able to QA a change by driving a real browser through the app — preflight, walk the flows, report what actually renders?" Options: `Yes — enable`, `No — skip`. Recommend `Yes` for any project with a user-facing frontend, especially one where `run_options.run_e2e` is off (nothing else exercises the browser). Requires a browser skill or MCP in the harness; the skill refuses to run without one. | Render [resources/dev-desk-check-template.md](resources/dev-desk-check-template.md) using `skills.dev-desk-check.*` (see the bootstrap follow-ups). Strip the services region when `services` is absent, the feature-flag region when `feature_flags` is absent or `system: none`, and the design region when `design_source` is absent or `type: none` — heading included, so the section disappears rather than shipping empty. |
| `qa-review` | "Do product managers or designers on this project need to QA a feature themselves, without a code checkout?" Options: `Yes — enable`, `No — skip`. Independent of `dev-desk-check` — asked separately, recorded separately. | Render [resources/qa-review-template.md](resources/qa-review-template.md) from the same `skills.dev-desk-check.*` block. **Then run the portability check before saving** — see the validation paragraph below. |
...
7. **`write-unit-test` (when shipped)** renders with no surviving `{{PLACEHOLDER}}`, cites the project's real test command, and every pack path in its `{{FRAMEWORK_PACK_LOADER_BLOCK}}` resolves to a file that actually landed under `ai-tools/skills/write-unit-test/resources/packs/`. One runner pack; stack packs only for frameworks the project actually uses (no `django.md` in a FastAPI repo, no web-framework pack in a pure-library repo). **Medplum projects**: confirm both FHIR-indexing setup files exist and are registered in `vitest.config.*` (`test.globalSetup` + `test.setupFiles`), `@medplum/definitions` is a devDependency, and no test re-indexes.
8. Setup script ([vinta-install-ai-tools-setup](../vinta-install-ai-tools-setup/SKILL.md)) runs cleanly with the new skills in place.
