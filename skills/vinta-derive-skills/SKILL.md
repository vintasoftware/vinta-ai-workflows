...
The jargon grep's term set is wider here than in Task 3's template-only check, on purpose: `ledger`, `seed`, `pull request` and `branch` are added because three of the blocks that feed this render — `{{QA_WRITE_POLICY_BLOCK}}`, `{{QA_DATA_SENSITIVITY_BLOCK}}`, `{{QA_ARTIFACT_DELIVERABLE_BLOCK}}` — are shared with the engineer-facing `dev-desk-check` path. Jargon can arrive **through a rendered block** even when the template itself is clean, which is exactly what a template-only grep (Task 3 Step 10, which only ever sees `{{...}}` still unexpanded) cannot catch. This derive-time check is where a config value that leaked engineer language into a shared block actually gets caught.
