...
| `{{QA_PREFLIGHT_BLOCK}}` | `environments.<name>.{api_url,api_health_path,frontend_start,edge_proxy}` (qa-frontend only) | Per-environment preflight steps in the fixed order **services → frontend → backend**. Emit a `curl -sS -o /dev/null -w '%{http_code}' --max-time 10 <api_url><api_health_path>` probe **only** when `api_url` is set **and** `edge_proxy` is **absent**; when `edge_proxy` is set, emit instead: `<name> sits behind <edge_proxy>, which answers non-browser traffic with a 403. No curl preflight — the browser is the preflight.` Emit the frontend-start offer only when `frontend_start` is set; otherwise emit `No start command is declared for <name>. Probe <base_url> and stop — never start a server. A second dev server on an occupied port silently invalidates the whole run.` |
| `{{QA_SERVICES_BLOCK}}` | `skills.qa-frontend.services[]` (qa-frontend only) | The whole `## Services pre-check` section body: one bullet per service with its `probe`, its `required` verdict (`true` → stop the run and offer `start`; `false` → continue and record the degradation in the report), and the three fixed rules (probe-cannot-run is `unknown` not `down`; auto-start is local-only and asks first; probes run before the frontend and backend checks). **Empty string when `services` is absent or empty — the whole section disappears, heading included.** |
| `{{QA_WRITE_POLICY_BLOCK}}` | `environments.<name>.writes` (qa-frontend + qa-review) | Per-environment write rules plus **all seven** fixed rules from the spec's **Production write gate** — both variants render all seven, never a trimmed subset: (1) the confirmation must name the environment; (2) `free` still keeps the ledger; (3) the ledger is written incrementally; (4) **an unlisted write stops the run for its own confirmation** — reversals are pre-authorised; (5) `forbidden` refuses and proposes; (6) **writes carry synthetic values only — never real client or patient data as input; under `phi`, the ledger records opaque IDs, not field values**; (7) setup writes (feature flags, seeds, migrations) stay propose-only in every environment. In the `qa-review` render, additionally append the UI-reversal rule — a write is only offered when its reversal is achievable through the app's own screens — and render all seven in **reviewer language**, avoiding `ledger`, `seed`, `migration`, `pull request` and `branch` (e.g. rule 3 becomes "kept as you go, not written at the end"; rule 7 talks about "a change whose effects reach beyond the one thing you're testing" rather than naming feature flags / seeds / migrations by their engineering terms). |
