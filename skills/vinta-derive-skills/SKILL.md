...
description: Author project-specific skills under `ai-tools/skills/`. Always copies the project-agnostic foundation set (`plan-feature`, `create-spec`, `open-pr-from-context`) verbatim from this skill's bundled resources; ships `create-qa-use-cases` and keeps `plan-feature`'s e2e sections only when `add-e2e-test` is enabled. Generates `implement-plan` from a parameterized template using project-specific commands + branch / commit / PR conventions captured in the bootstrap interview. Asks the user whether `add-e2e-test`, `add-env-var`, `qa-frontend` and `qa-review` are needed (optional foundation skills — only ship if applicable). Stack-specific skills (Medplum, Django, etc) come from user-supplied templates pointed at by the orchestrator — see [bootstrap-ai-tools/resources/stacks/<stack>/notes.md](../vinta-bootstrap-ai-tools/resources/stacks/) for the categories per stack.
---
...
- **Replace with Vinta foundation version** (foundation-shape only — `plan-feature`, `create-spec`, `create-qa-use-cases`, `implement-plan`, `add-e2e-test`, `add-env-var`, `add-one-off-script`, `prepare-worktree`, `thermo-nuclear-code-quality-review`, `deslop-comments`, `handoff`, `handoff-to-client`, `write-unit-test`, `qa-frontend`, `qa-review`) — proceed with the bucket A / B / C flow below for that name. The user has explicitly opted into overwriting their version.
...
| `{{QA_ESCALATION_CONTACT}}` | `skills.qa-frontend.escalation_contact` (qa-frontend + qa-review) | The declared contact verbatim. Renders `the engineer who gave you this file` when absent. |
...
- `ai-tools/skills/qa-frontend/SKILL.md` (from `qa-frontend-template.md`) — only when `foundation_skills.qa-frontend: enabled`
...
| `qa-frontend` | "Should agents be able to QA a change by driving a real browser through the app — preflight, walk the flows, report what actually renders?" Options: `Yes — enable`, `No — skip`. Recommend `Yes` for any project with a user-facing frontend, especially one where `run_options.run_e2e` is off (nothing else exercises the browser). Requires a browser skill or MCP in the harness; the skill refuses to run without one. | Render [resources/qa-frontend-template.md](resources/qa-frontend-template.md) using `skills.qa-frontend.*` (see the bootstrap follow-ups). Strip the services region when `services` is absent, the feature-flag region when `feature_flags` is absent or `system: none`, and the design region when `design_source` is absent or `type: none` — heading included, so the section disappears rather than shipping empty. |
| `qa-review` | "Do product managers or designers on this project need to QA a feature themselves, without a code checkout?" Options: `Yes — enable`, `No — skip`. Independent of `qa-frontend` — asked separately, recorded separately. | Render [resources/qa-review-template.md](resources/qa-review-template.md) from the same `skills.qa-frontend.*` block. **Then run the portability check before saving** — see the validation paragraph below. |
