...
3. Inner loop: `{{LINT_CMD}}` → {{NEW_TEST_CMD_PATTERN}} → scoped suite ({{SCOPED_TEST_PATTERN}}).
4. Iterate 2–3 until green.
5. Outer gate: `{{BUILD_CMD}}` AND `{{TEST_CMD}}`{{E2E_OUTER_GATE_LINE}}.
6. Stage explicitly: `git add {{STAGE_PATTERN}}`.
7. Commit. {{COMMIT_STYLE_LINE}}.
8. {{COAUTHOR_INSTRUCTION_LINE}}
