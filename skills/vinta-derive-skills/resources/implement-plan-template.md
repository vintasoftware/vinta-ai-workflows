...
3. **Run prepare-worktree.** Pass the plan file + worktree name. The skill returns:
   ...
   - `sandbox_tier` — `enforced` (the [Filesystem sandbox](../prepare-worktree/SKILL.md#step-55--filesystem-sandbox-os-level-write-guard) step will block main-checkout writes: `sandbox-exec` / `bwrap` for subprocess runtimes, or the generated `settings.json` `PreToolUse` guard [+ optional native sandbox] for in-process runtimes like claude-code) or `none` (no guard wired — prevention degrades to the Layer 1 backstop). Drives the [Spawn subagent](#1c-spawn-subagent) wrapping + Layer 1 below.
...
`<main-checkout-root>` is the repo root the skill was invoked from (never `worktree_path`). A stray write then fails with `Operation not permitted` / `EROFS`; the subagent retries against the worktree. **`--allow <main-checkout-root>/.git` is required** — git worktrees write every commit into the main repo's `.git` (`.git/worktrees/<name>/index.lock`, shared objects/refs), so a deny-main that omits it makes the subagent's own commit fail with `Operation not permitted`.

- **In-process subagent runtimes** (the orchestrator and subagent share one OS process — e.g. **claude-code's Task tool**) can't wrap a single spawn: there's no child launch command to put behind `sandbox-run.sh`. Use the harness-config guard [prepare-worktree](../prepare-worktree/SKILL.md#55a--in-process-runtimes-claude-code) generates instead. In the [Provision worktree](#step-05--provision-worktree-when-run_optionsuse_worktree--true) step, run `ai-tools/skills/prepare-worktree/scripts/gen-claude-sandbox-settings.sh --worktree <worktree_path> --main <main-checkout-root>` **before** the plan session starts. It wires two layers into the worktree's `settings.json`, both covering the orchestrator AND every in-process Task subagent (they share the session pipeline, so a session-level `PreToolUse` hook fires for both):
  - **Layer A (always) — a `PreToolUse` write-guard hook** (`ai-tools/skills/prepare-worktree/scripts/claude-worktree-write-guard.py`) that blocks the file-editing tools (`Edit`/`Write`/`MultiEdit`/`NotebookEdit`) from writing outside the worktree. Filesystem-only, no network impact. Record `sandbox_tier = enforced` (or keep it as the file-tool guard when the team declines Layer B).
  - **Layer B (opt-in, `--os-sandbox`) — Claude Code's native OS sandbox** (`sandbox.filesystem.denyWrite`/`allowWrite`), which also blocks Bash-issued writes inside subagents. It forces network isolation, so pass the registry + git-remote hosts via `--allow-domain`; without Layer B, Bash-issued stray writes fall back to the Layer 1 backstop below.

  Because these settings are read at **session start**, provision the worktree + generate the settings before launching the plan session (ideally rooted in the worktree: `cd <worktree_path> && claude`). Don't expect a mid-run `use_worktree` opt-in to retroactively cage the already-running session's Bash sandbox.
- **`run_options.sandbox_tier = none`** (no sandbox tool on the machine, and — for claude-code — the guard hook not wired) → skip wrapping; prevention falls back entirely to the Layer 1 stray-write check below. Surface this once to the user so the weaker guarantee is explicit.
...
- **Prevent stray main-checkout writes at the OS layer when you can; catch them always.** When `use_worktree = true` and the runtime spawns subagents as **subprocesses**, wrap every spawn in `prepare-worktree`'s `sandbox-run.sh` (deny main checkout, allow worktree + `<main>/.git` + `.vinta-ai-workflows`). For **in-process** runtimes (claude-code's Task tool) there's no spawn to wrap: generate the worktree `settings.json` guard with `gen-claude-sandbox-settings.sh` (a `PreToolUse` hook that fires for the orchestrator and every Task subagent, plus optional native OS sandbox) before the session starts. Either way main-checkout writes are blocked regardless of harness — see [Spawn subagent](#1c-spawn-subagent). This is *prevention*; the prompt instruction alone is not. Never omit `<main>/.git` from the allow-set — worktree commits write there. Regardless of tier, Layer 1 runs `git -C <main-checkout-path> status --short` after every implementer and fixer as the backstop; any tracked modification is a BLOCKER — recover the intent, re-dispatch to the worktree, then restore the main checkout clean. `<main-checkout-path>` is the repo root the skill was invoked from, never `run_options.worktree_path`.
...
- [ ] Subagent spawned, report received. When `use_worktree = true` AND `sandbox_tier = enforced`: subprocess spawn wrapped in `sandbox-run.sh` (deny main, allow worktree + `<main>/.git` + `.vinta-ai-workflows`), or — for in-process runtimes (claude-code) — the session's `settings.json` guard from `gen-claude-sandbox-settings.sh` is active.
