...
> **Optional foundation skills are gated by the bootstrap interview.** `systematic-debugging`, `add-e2e-test`, `add-env-var`, `add-one-off-script`, `prepare-worktree`, `thermo-nuclear-code-quality-review`, `qa-frontend`, `qa-review`, and `handoff-to-client` ship **only** if the user says yes during `vinta-bootstrap-ai-tools` Step 0 (or later opts in via `vinta-sync-ai-tools`). They are not in every bootstrapped project. The opt-in is recorded in `.vinta-ai-workflows.yaml` under `foundation_skills.*.enabled` and is sticky across syncs.
>
...
| `qa-frontend` | **opt-in** | Drive a real browser through the app as a user and report what actually renders — the manual pass that complements automated e2e. Plan source is the branch diff, ids from `QA_USE_CASES.md`, or a ticket. Preflights services / frontend / backend per environment, enforces the project's per-environment write policy and data-sensitivity rules before opening the browser, keeps an incremental write ledger with a reversal per entry, and writes an evidence-backed report plus a PR `Q.A.` section. Template-rendered from `skills.qa-frontend.*` config. |
| `qa-review` | **opt-in** | The same browser walk and report shape for product managers and designers, with git, diffs, dev servers, service probes, PR edits and database access stripped out. Rendered **self-contained** — no relative links, no repo-dependent commands — so an engineer can hand over the single `SKILL.md` and it works with no checkout. Reads the same `skills.qa-frontend.*` config block; opt-in independently of `qa-frontend`. |
