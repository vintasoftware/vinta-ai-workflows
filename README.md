...
| [`deslop-comments`](skills/vinta-derive-skills/resources/foundation-skills/deslop-comments/SKILL.md) | always | Rewrite comments + doc blocks touched during a task into Simple English (strips AI-slop vocabulary + negative framing; text-only — no renames, no behavior change). Also accepts a prose file as an explicit scope, which is how `integrate-phase` / `amend-plan` keep PR titles, descriptions, and inline comments in Simple English. `review-phase` Layer 2 depends on it; also invokable standalone. |
